...
public:
    ...
    Account(string sur, int acc, float per, float sum) : surname(sur), accNum(acc), percent(per), summa(sum) {
        cout << "Вызов конструктора c 4 объектами " << this << endl;
    }

    ~Account() { cout << "Вызов деструктора объекта " << this << endl; }
...
public:
    void setAccount(string surname, int accNum, float percent, float summa) {
...
{
    setlocale(LC_ALL, "ru");
    Account bill("Gotin", 5555, 2.5, 5000.24);
    bill.Print();
    Account bill_2;
    bill_2.setAccount("Myrov", 5555, 2.5, 5000.24);
